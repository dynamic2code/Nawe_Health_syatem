...
var parseGeoHeader = (geoHeader) => {
  ...
  try {
    const geoData = JSON.parse(Buffer2.from(geoHeader, "base64").toString("utf-8"));
    return geoData;
  } catch {
    return {};
  }
...
var addHeaders = (request) => {
  const store = requestStore.getStore();
  if (store == null ? void 0 : store.cdnLoopHeader) {
    request.setHeader("CDN-Loop", store == null ? void 0 : store.cdnLoopHeader);
  }
};
var startInterceptor = () => {
  ...
  globalThis.fetch = function patchedFetch(input, init) {
    ...
    const store = requestStore.getStore();
    if (store == null ? void 0 : store.cdnLoopHeader) {
      request.headers.set("CDN-Loop", store.cdnLoopHeader);
    }
    return originalFetch(request);
  };
  for (const lib of [http, https]) {
    const originalRequest = lib.request;
    lib.get = function patchedGet(...args) {
      const request = originalRequest(...args);
      addHeaders(request);
      request.end();
      return request;
    };
    lib.request = function patchedRequest(...args) {
      const request = originalRequest(...args);
      addHeaders(request);
      return request;
    };
...
var getLambdaHandler = (func) => awslambda.streamifyResponse(async (event, responseStream, lambdaContext) => {
  ...
  if (featureFlags.get("serverless_functions_request_interceptor_v2") === true) {
    startInterceptor();
  }
  ...
  const res = await requestStore.run({ cdnLoopHeader: headers.get("cdn-loop") }, () => func.default(req, context));
  const invocationMetadata = {
    version
  };
  const invocationMetadataHeader = Buffer4.from(JSON.stringify(invocationMetadata)).toString("base64");
