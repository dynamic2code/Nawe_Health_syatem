...
const createInternalContext = async (moduleOptions, nuxt = useNuxt()) => {
  ...
  const trackProxy = (configPath, path = []) => ({
    get: (target, key) => {
      return typeof target[key] === "object" && target[key] !== null ? new Proxy(target[key], trackProxy(configPath, path.concat(key))) : target[key];
    },
    set(target, key, value) {
      const resultingCode = `cfg${path.concat(key).map((k) => `[${JSON.stringify(k)}]`).join("")} = ${JSON.stringify(value)};`;
      if (JSON.stringify(target[key]) === JSON.stringify(value) || configUpdatedHook[configPath].endsWith(resultingCode)) {
  ...
  const loadConfig = async () => {
    ...
    const tailwindConfig = await Promise.all(
      configPaths.map(async (configPath, idx, paths) => {
        let _tailwindConfig;
        try {
          _tailwindConfig = configMerger(void 0, _loadConfig(configPath));
        } catch (e) {
  ...
  async setup(moduleOptions, nuxt) {
    ...
    nuxt.hook("modules:done", async () => {
      ...
      postcssOptions.plugins = {
        ...
        "tailwindcss": enableHMR ? twConfig.dst : _config
      };
      if (nuxt.options.dev) {
